...
- **player**（`core/`、`net/`、`select_loop/`、`device/`、`rules/`、`bridge/`、`music_source/`）：`core/player_constants.h`、`core/player_types.h`、`core/player.h`、`core/player.c`、`core/player_fifo.c`（init_*_fifo）、`core/player_gst.c`、`select_loop/select.c`、`select_loop/select_text.c`、`select_loop/select_text.h`、`select_loop/select_music_llm.c`、`select_loop/select_music_llm.h`、`net/socket.c`、`net/socket_report.c`、`net/socket_report.h`、`net/link.c`、`device/device.c`、`core/shm.c`、`rules/rule_match.c`、`bridge/music_lib_bridge.c`、`core/main.c`。
...
- `smart-speaker-client/player/core/player.c`
  - 在线/离线模式切换改为通过 `asr_ctrl_fifo` 通知 ASR。
  - 新增 SD 卡 `mmcblk*p*` 自动探测并挂载。

- `smart-speaker-client/player/core/main.c`
